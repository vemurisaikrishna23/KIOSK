...
/**
 * User Activity — view & download the audit trail for a single account
 * (the actions that user performed) over a selectable window (default today,
 * up to 30 days).
 *
 * Permissions:
 *   – user_activity_view      → view this page
 *   – user_activity_download  → export the visible range as CSV
 */
export default function UserActivity() {
  ...
  const canView     = auth.hasPerm('user_activity_view')
  const canDownload = auth.hasPerm('user_activity_download')
  ...
  const load = useCallback(async () => {
    ...
    try {
      const resp = await api.listUserActivity({ userId, ...rangeParams(), page, page_size: pageSize })
      setLogs(Array.isArray(resp?.logs) ? resp.logs : [])
      if (resp?.user) setAccount(resp.user)
      setTotal(resp?.count || 0)
      setTotalPages(resp?.total_pages || 1)
    } catch (e) {
      setError(
        e?.status === 404 ? 'User not found.'
          : e?.network ? 'Could not reach the server.'
          : (e?.message || 'Failed to load user activity.'),
      )
    } finally {
      setLoading(false)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [canView, userId, page, pageSize, range, customStart, customEnd, category, customInvalid])
  ...
  async function onDownload() {
    ...
    setDownloading(true)
    try {
      const csv = await api.downloadUserActivity({ userId, ...rangeParams() })
      const blob = new Blob([typeof csv === 'string' ? csv : ''], { type: 'text/csv;charset=utf-8;' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      const who = (account?.name || account?.email || `user-${userId}`).replace(/[^a-z0-9]+/gi, '-')
      const tag = range === 'custom' ? `${customStart}_to_${customEnd}` : `last-${range}d`
      a.download = `user-activity_${who}_${tag}.csv`
  ...
  if (!canView) {
    return (
      <div className="kiosk-app">
        <TopBar />
        <div className="admin-page"><PermissionDenied resource="user activity" /></div>
      </div>
    )
  }
  ...
  const who = account?.name || account?.email || `User #${userId}`
    ...
    <div className="kiosk-app kiosk-app-fixed">
      ...
      <div className="admin-page alog-page">
        <header className="admin-page-head">
          <div>
            <button type="button" className="link-btn alog-back" onClick={() => navigate('/users')}>← Back to Users</button>
            <h1>Activity · {who}</h1>
            <p className="lede">
              Everything {account?.name ? account.name : 'this account'} changed — who did what, kept for 30 days.
              {account?.email ? <span className="user-sub"> · {account.email}</span> : null}
            </p>
          </div>
        ...
        <section className="admin-card list-card alog-table">
          ...
          <div className="list-scroll">
            {loading ? (
              ...
              <div className="admin-empty">No activity by this user in this period.</div>
            ) : (
              logs.map((r) => {
